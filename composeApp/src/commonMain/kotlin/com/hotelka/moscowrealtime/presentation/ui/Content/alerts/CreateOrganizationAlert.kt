...
import moscowrealtime.composeapp.generated.resources.error_creating_organization
import moscowrealtime.composeapp.generated.resources.found_organizations_alike
import moscowrealtime.composeapp.generated.resources.organization
...
fun CreateOrganizationAlert(
    ...
    NeutralAlert(
        titleText = stringResource(Res.string.create_organization),
        secondaryText = stringResource(Res.string.provide_organization_name),
        cancelText = stringResource(Res.string.cancel),
        submitText = stringResource(Res.string.create_organization),
        onSubmit = { onCreateOrganization(organizationName); onDismissDialog() },
        onDismissRequest = onDismissDialog,
        middleContent = {
            CustomTextField(
                value = organizationName,
                onValueChange = {
                    organizationName = it; showCreateError = false; showCheckError = false
                },
                isError = showCheckError || similarOrganizations.isNotEmpty(),
                errorDescription = {
                    if (showCheckError) {
                    ...
                    } else if (showCreateError) {
                        Text(stringResource(Res.string.error_creating_organization))
                    }
                },
                leadingIcon = {
                    Icon(painterResource(Res.drawable.briefcase), "BriefCase")
                },
                modifier = Modifier.fillMaxWidth(),
                label = {
                    Text(stringResource(Res.string.organization))
                },
